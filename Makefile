...
UPSTREAM_IMPORT_PATH=$(GOPATH)/src/github.com/nholuongut/kube-router/
...
# https://github.com/nholuongut/kube-router/issues/1676
...
ifeq "$(BUILD_IN_DOCKER)" "true"
	$(DOCKER) run -v $(PWD):/go/src/github.com/nholuongut/kube-router \
		-v $(GO_CACHE):/root/.cache/go-build \
		-v $(GO_MOD_CACHE):/go/pkg/mod \
		-w /go/src/github.com/nholuongut/kube-router $(DOCKER_BUILD_IMAGE) \
		sh -c \
		'GOARCH=$(GOARCH) CGO_ENABLED=0 go build \
		-ldflags "-X github.com/nholuongut/kube-router/v2/pkg/version.Version=$(GIT_COMMIT) -X github.com/nholuongut/kube-router/v2/pkg/version.BuildDate=$(BUILD_DATE)" \
...
ifeq "$(BUILD_IN_DOCKER)" "true"
	$(DOCKER) run -v $(PWD):/go/src/github.com/nholuongut/kube-router \
		...
		-w /go/src/github.com/nholuongut/kube-router $(DOCKER_BUILD_IMAGE) \
		sh -c \
		'CGO_ENABLED=0 go test -v -timeout 30s github.com/nholuongut/kube-router/v2/cmd/kube-router/ github.com/nholuongut/kube-router/v2/pkg/...'
else
	go test -v -timeout 30s github.com/nholuongut/kube-router/v2/cmd/kube-router/ github.com/nholuongut/kube-router/v2/pkg/...
endif
...
ifeq "$(BUILD_IN_DOCKER)" "true"
	$(DOCKER) run -v $(PWD):/go/src/github.com/nholuongut/kube-router \
		-v $(GO_CACHE):/root/.cache/go-build \
		-v $(GO_MOD_CACHE):/go/pkg/mod \
		-w /go/src/github.com/nholuongut/kube-router $(DOCKER_LINT_IMAGE) \
		bash -c \
		'golangci-lint run ./...'
else
	golangci-lint run ./...
